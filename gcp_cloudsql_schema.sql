-- =====================================================================
-- SEAFUDZ NG BAYAN - GOOGLE CLOUD SQL (POSTGRESQL) SCHEMA
-- Architecture: Supabase (Auth Only) + GCP Server (Express Backend) + Cloud SQL Postgres
-- Updates: Explicit Assistant & Payment Handling Integration
-- =====================================================================

-- 1. EXTENSIONS
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 2. DROP TABLES IF THEY EXIST (Safe Re-run)
...
-- EMPLOYEES TABLE (Staff & Admin Profiles linked to Supabase Auth UID)
CREATE TABLE employees (
    ...
    pin_code VARCHAR(10), -- 4-digit PIN for POS counter terminal fast login
...
-- CUSTOMERS TABLE (Customer Profiles linked to Supabase Auth UID)
...
-- ORDERS TABLE (Links Table, Customer, Cashier, Assistant, and Rider)
CREATE TABLE orders (
    id VARCHAR(100) PRIMARY KEY,
    ...
    assistant_id UUID REFERENCES employees(id) ON DELETE SET NULL, -- Assistant who handled payment/bill at table
...
-- ORDER ITEMS TABLE (Financial Audit Snapshot Pattern)
...
-- ASSISTANT / FLOOR CALLS TABLE
CREATE TABLE assistant_calls (
    id VARCHAR(100) PRIMARY KEY,
    table_id VARCHAR(100) REFERENCES tables(id) ON DELETE CASCADE,
    assistant_id UUID REFERENCES employees(id) ON DELETE SET NULL, -- Assistant assigned to call
    type VARCHAR(100) NOT NULL DEFAULT 'Call Waiter', -- 'Call Waiter', 'Water Refill', 'Request Bill', 'Clean Table'
    status VARCHAR(50) NOT NULL DEFAULT 'Pending',
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- ---------------------------------------------------------------------
-- 4. INDEXES FOR GCP CLOUD SQL PERFORMANCE
-- ---------------------------------------------------------------------
CREATE INDEX idx_employees_supabase_user_id ON employees(supabase_user_id);
CREATE INDEX idx_employees_role ON employees(role);
CREATE INDEX idx_customers_supabase_user_id ON customers(supabase_user_id);
...
CREATE INDEX idx_assistant_calls_table ON assistant_calls(table_id);
CREATE INDEX idx_assistant_calls_assistant ON assistant_calls(assistant_id);
...
-- ---------------------------------------------------------------------
-- 6. SEED DATA FOR GCP CLOUD SQL
-- ---------------------------------------------------------------------

-- Employees Seed (Each mapped to a mock Supabase Auth UID)
...
-- Assistant Floor Calls Seed (Linked to Assistant Grace - EMP-004)
...
-- Sample Orders Seed (Linked to Assistant Grace - EMP-004 for table bill settlement)
...
INSERT INTO order_items (order_id, menu_item_id, snapshot_item_name, unit_price, quantity) VALUES
('ORD-1001', 'item-001', 'Seafood Bilao Feast', 2400.00, 1);
